...
" Collection of user plugin list config file-path
...
function! s:load_plugins(plugin_files)
	...
	if filereadable(a:plugin_files)
		...
		if empty(content)
			...
			call filter(s:config_paths, 'fileradable(v:val)')
...
function! s:load_config(filename)
	...
	elseif a:filename =~# '\.ya\?ml$'
		...
		let l:yaml_content = s:load_yaml(a:filename)
		return empty(l:yaml_content)?[]:l:yaml_content
	endif
